int main()
{
    ...
    std::cout << "System time at start(in seconds past midnight): " << system_time_starts << std::endl;

    timer.wait_for_few_seconds();  
    timer.stop_time(); 
    std::cout << "Stopped Timer" << std::endl;
    long system_time_stops = timer.get_the_system_time(); 
    std::cout << "System time at stop(in seconds past midnight): " << system_time_stops << std::endl;
